...
local function build_choice(rank, if0, if1)
   ...
   local node
   local function do_choose(if0, if1)
      if optimize then
...
local function subst(rank, replacement, node)
   ...
   elseif rank == node.rank then return replacement.choose(node.if0, node.if1)
...
local function really_choose(node, if0, if1)
   ...
   local on0 = subst(top, lit0, node).choose(subst(top, lit0, if0),
                                             subst(top, lit0, if1))
   local on1 = subst(top, lit1, node).choose(subst(top, lit1, if0),
                                             subst(top, lit1, if1))
...
local function bdd_and(node1, node2) return node2.choose(lit0, node1) end
-- local function bdd_and(node1, node2) return node1.choose(lit0, node2) end
local function bdd_or (node1, node2) return node2.choose(node1, lit1) end
