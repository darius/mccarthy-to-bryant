-- Like bdd4.lua but with a less-general slightly-faster subst.
...
local function subst(rank, value, node)
   local node_rank, if0, if1 = unpack(nodes[node])
   if rank < node_rank then
      return node
   elseif rank == node_rank then
      if value == 0 then return if0
                    else return if1 end
   else
      return make_choice(node_rank,
                         subst(rank, value, if0),
                         subst(rank, value, if1))
   end
end
...
local function really_choose(node, if0, if1)
   ...
   local on0 = do_choose(subst(top, 0, node), subst(top, 0, if0),
                                              subst(top, 0, if1))
   local on1 = do_choose(subst(top, 1, node), subst(top, 1, if0),
                                              subst(top, 1, if1))
